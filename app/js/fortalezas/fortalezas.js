		...
		function load(page){
			...
			$("#loader").fadeIn('slow');
			$.ajax({
				url:'ajax/fortalezas/listar.php',
				data: parametros,
		...
		$("#xpdf").on('click', function(event){			
			...
			doc.autoTable({ 
				...
				columns: [
					{ header: ' ', dataKey: 'no' },
					{ header: 'Nombre', dataKey: 'CAT_Nombre' },
				],
				...
				didDrawPage: function (data) {					
					...
					doc.setFontSize(20);
					doc.setTextColor(40);
					doc.text("Reporte de Fortalezas", data.settings.margin.left+190, 40);
			...
			doc.save('fortalezas.pdf')
		})
		
		$( "#addFortalezasModal" ).on('show.bs.modal', function () {			
			setTimeout(function (){
				$('#Name2').focus();
			}, 1000)
		});
		...
		$( "#add_fortalezas" ).submit(function( event ) {
			...
			$.ajax({
				type: "POST",
				url: "ajax/fortalezas/guardar.php",
				data: parametros,
				beforeSend: function(objeto){
		...
		$('#editFortalezasModal').on('show.bs.modal', function (event) {
			setTimeout(function (){
				$('#edit_name').focus();
			}, 1000)
		...
		$( "#edit_fortalezas" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "ajax/fortalezas/editar.php",
				data: parametros,
				beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
				success: function(datos){
					m = datos.trim();	
					$("#resultados").html(datos);
					$('#editFortalezasModal').modal('hide');
					let msj = m.substr(0,1);
					let type;
					let txt;
					if(msj == 'U'){
						type = 'success';
						txt = 'Fortaleza ha sido actualizada con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
						txt = 'Ya existe un Registro grabado con el mismo Nombre.';
		...
		$('#deleteFortalezasModal').on('show.bs.modal', function (event) {
		  	var button = $(event.relatedTarget) // Button that triggered the modal
		  	var id = button.data('id') 
		  	$('#delete_id').val(id)
		})
		
		$( "#delete_fortalezas" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "ajax/fortalezas/delete.php",
				data: parametros,
				beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
				success: function(datos){
					let m= datos.trim();
					$("#resultados").html(datos);
					$('#deleteFortalezasModal').modal('hide');
					let msj = m.substr(0,1);
					let type;
					let txt;
					if(msj == 'B'){
						type = 'success';
						txt = 'Fortaleza ha sido borrada con éxito.';
					}
					else if(msj == 'R'){
						type= 'error';
