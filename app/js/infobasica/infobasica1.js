		...
		function load(page){
			//var query=$("#q").val();
			//var per_page=10;
			//var parametros = {"action":"ajax","page":page,'query':query,'per_page':per_page, "ruta":"../"};
			var parametros = {"action":"ajax", "ruta":"../"};
			$("#loader").fadeIn('slow');
			$.ajax({
		...
		$("#xpdf").on('click', function(event){			
			...
			doc.autoTable({ 
				...
				columns: [
					{ header: ' ', dataKey: 'no' },
					{ header: 'Nombre', dataKey: 'ProcesosName' },
				],
				...
				didDrawPage: function (data) {					
					...
					doc.setFontSize(20);
					doc.setTextColor(40);
					doc.text("Reporte de Procesos", data.settings.margin.left+190, 40);
		  ...
		  var parametros = $(this).serialize();
			$.ajax({
					...
					success: function(datos){
						...
						if(msj == 'O'){
							type = 'success';
							txt = 'Info Básica ha sido guardada con éxito.';
						}
						else if(msj == 'E'){
							type= 'warning';
