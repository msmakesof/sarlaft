		...
		function load(page){
			...
			$("#loader").fadeIn('slow');
			$.ajax({
				url:'ajax/listar_segproductos.php',
				data: parametros,
		...
		$("#xpdf").on('click', function(event){			
			...
			doc.autoTable({ 
				...
				columns: [
					{ header: ' ', dataKey: 'no' },
					{ header: 'Nombre', dataKey: 'SegProductosName' },
				],
				...
				didDrawPage: function (data) {					
					...
					doc.setFontSize(20);
					doc.setTextColor(40);
					doc.text("Reporte Segmento Productos", data.settings.margin.left+190, 40);
			...
			doc.save('segmentoproductos.pdf')
		})
		...
		$('#editSegProductosModal').on('show.bs.modal', function (event) {
			setTimeout(function (){
				$('#edit_name').focus()
			}, 1500)
		...
		$('#deleteSegProductosModal').on('show.bs.modal', function (event) {
		  var button = $(event.relatedTarget) // Button that triggered the modal
		  var id = button.data('id') 
		  $('#delete_id').val(id)
		})
		...
		$( "#edit_segproductos" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
					type: "POST",
					url: "ajax/editar_segproductos.php",
					data: parametros,
					 beforeSend: function(objeto){
					...
					$('#editSegProductosModal').modal('hide');
					setTimeout(function (){
						load(1);location.reload();
					}, 3000)
				  }
		...
		$( "#delete_segproductos" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
					type: "POST",
					url: "ajax/delete_segproductos.php",
					data: parametros,
					 beforeSend: function(objeto){
						$("#resultados").html("Enviando...");
					  },
					...
					$("#resultados").html(datos);
					//load(1);location.reload();
					$('#deleteSegProductosModal').modal('hide');
					setTimeout(function (){
						load(1);location.reload();
					}, 3000)
				  }
