		...
		function load(page){
			...
			$("#loader").fadeIn('slow');
			$.ajax({
				url:'ajax/factoresriesgo/listar.php',
				data: parametros,
		...
		$("#xpdf").on('click', function(event){			
			...
			doc.autoTable({ 
				...
				didDrawPage: function (data) {					
					...
					doc.setFontSize(20);
					doc.setTextColor(40);
					doc.text("Reporte de Factores de Riesgo", data.settings.margin.left+190, 40);
		...
		$( "#edit_user" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "ajax/factoresriesgo/editar.php",
				data: parametros,
				beforeSend: function(objeto){
				...
				success: function(datos){
					...
					if(msj == 'U'){
						type = 'success';
						txt = 'Factor Riesgo ha sido actualizado con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
		...
		$( "#add_debilidades" ).submit(function( event ) {
			...
			$.ajax({
				type: "POST",
				url: "ajax/factoresriesgo/guardar.php",
				data: parametros,
				beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
		...
		$( "#delete_user" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "ajax/factoresriesgo/delete.php",
				data: parametros,
				beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
