		...
		function load(page){
			...
			$("#loader").fadeIn('slow');
			$.ajax({
				url:'ajax/nivelriesgo/listar.php',
				data: parametros,
		...
		$("#xpdf").on('click', function(event){		
			...
			doc.autoTable({ 
				...
				columns: [
					{ header: ' ', dataKey: 'no' },
					{ header: 'Nombre', dataKey: 'NIR_Nombre' },
				...
				didDrawPage: function (data) {					
					...
					doc.setFontSize(20);
					doc.setTextColor(40);
					doc.text("Reporte de Nivel der Riesgo", data.settings.margin.left+190, 40);
		...
		$( "#edit_user" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "ajax/nivelriesgo/editar.php",
				data: parametros,
				beforeSend: function(objeto){
				...
				success: function(datos){
					...
					if(msj == 'U'){
						type = 'success';
						txt = 'Nivel de Riesgo ha sido actualizado con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
		...
		$( "#add_debilidades" ).submit(function( event ) {
			...
			$.ajax({
				type: "POST",
				url: "ajax/nivelriesgo/guardar.php",
				data: parametros,
				beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
				success: function(datos){
					...
					if(msj == 'O'){
						type = 'success';
						txt = 'Nivel de Riesgo ha sido guardado con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
						txt = 'Ya existe un Registro grabado con el mismo Nombre.';
		...
		$( "#delete_user" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "ajax/nivelriesgo/delete.php",
				data: parametros,
				beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
				success: function(datos){
					...
					if(msj == 'B'){
						type = 'success';
						txt = 'Nivel de Riesgo ha sido borrado con éxito.';
					}
					else if(msj == 'R'){
						type= 'error';
