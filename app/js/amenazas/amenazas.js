		...
		$("#xpdf").on('click', function(event){			
			...
			doc.autoTable({ 
				...
				columns: [
					{ header: ' ', dataKey: 'no' },
					{ header: 'Nombre', dataKey: 'AmenazasName' },
				],
				...
				didDrawPage: function (data) {					
					...
					doc.setFontSize(20);
					doc.setTextColor(40);
					doc.text("Reporte de Amenazas", data.settings.margin.left+190, 40);
		  ...
		  var parametros = $(this).serialize();
			$.ajax({
				...
				success: function(datos){
					...
					if(msj == 'U'){
						type = 'success';
						txt = 'Amenaza ha sido actualizada con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
						txt = 'Ya existe un Registro grabado con el mismo Nombre.';
		  ...
		  var parametros = $(this).serialize();
			$.ajax({
				...
				success: function(datos){
					...
					if(msj == 'B'){
						type = 'success';
						txt = 'Amenaza ha sido borrada con éxito.';
					}
					else if(msj == 'R'){
						type= 'error';
