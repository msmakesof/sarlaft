		...
		function load(page){
			...
			$("#loader").fadeIn('slow');
			$.ajax({
				url:'ajax/tiposriesgo/listar.php',
				data: parametros,
		...
		$("#xpdf").on('click', function(event){			
			...
			doc.autoTable({ 
				...
				columns: [
					{ header: ' ', dataKey: 'no' },
					{ header: 'Nombre', dataKey: 'TIR_Nombre' },
				],
				...
				didDrawPage: function (data) {					
					...
					doc.setFontSize(20);
					doc.setTextColor(40);
					doc.text("Reporte de Tipos de Riesgo", data.settings.margin.left+190, 40);
		...
		$( "#edit_user" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "ajax/tiposriesgo/editar.php",
				data: parametros,
				beforeSend: function(objeto){
				...
				success: function(datos){
					...
					if(msj == 'U'){
						type = 'success';
						txt = 'Tipo Riesgo ha sido actualizado con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
		...
		$( "#add_debilidades" ).submit(function( event ) {
			...
			$.ajax({
				type: "POST",
				url: "ajax/tiposriesgo/guardar.php",
				data: parametros,
				beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
				success: function(datos){
					...
					if(msj == 'O'){
						type = 'success';
						txt = 'Tipo de Riesgo ha sido guardado con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
						txt = 'Ya existe un Registro grabado con el mismo Nombre.';
		...
		$( "#delete_user" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "ajax/tiposriesgo/delete.php",
				data: parametros,
				beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
				success: function(datos){
					...
					if(msj == 'B'){
						type = 'success';
						txt = 'Factor de Riesgo ha sido borrado con éxito.';
					}
					else if(msj == 'R'){
						type= 'error';
