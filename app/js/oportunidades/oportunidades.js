		...
		function load(page){
			...
			$("#loader").fadeIn('slow');
			$.ajax({
				url:'ajax/oportunidades/listar.php',
				data: parametros,
		...
		$("#xpdf").on('click', function(event){			
			...
			doc.autoTable({ 
				...
				didDrawPage: function (data) {					
					...
					doc.setFontSize(20);
					doc.setTextColor(40);
					doc.text("Reporte de Oportunidades", data.settings.margin.left+190, 40);
		...
		$( "#add_oportunidades" ).submit(function( event ) {
			...
			$.ajax({
				type: "POST",
				url: "ajax/oportunidades/guardar.php",
				data: parametros,
				beforeSend: function(objeto){
		...
		$( "#edit_oportunidad" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "ajax/oportunidades/editar.php",
				data: parametros,
				beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
				success: function(datos){
					...
					if(msj == 'U'){
						type = 'success';
						txt = 'Oportunidad ha sido actualizada con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
						txt = 'Ya existe un Registro grabado con el mismo Nombre.';
		...
		$( "#delete_oportunidades" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "ajax/oportunidades/delete.php",
				data: parametros,
				beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
				success: function(datos){
					...
					if(msj == 'B'){
						type = 'success';
						txt = 'Oportunidad ha sido borrada con éxito.';
					}
					else if(msj == 'R'){
						type= 'error';
