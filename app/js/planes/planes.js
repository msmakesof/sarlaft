		...
		function load(page){
			...
			$("#loader").fadeIn('slow');
			$.ajax({
				url:'ajax/planes/listar_planes.php',
				data: parametros,
		...
		$("#xpdf").on('click', function(event){			
			...
			doc.autoTable({ 
				...
				columns: [
					...
					{ header: 'Responsable', dataKey: 'NombreResponsable' },					
					{ header: 'Plazo', dataKey: 'PlanesPlazo' },
					{ header: 'Aprobado Por', dataKey: '$NombreResponsableApr' },
		...
		$( "#addTarea" ).submit(function( event ) {
			...
			$.ajax({
				type: "POST",
				url: "ajax/planes/guardar_tarea.php",
				data: parametros,
				 beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
				success: function(datos){
					...
					if(msj == 'O'){
						type = 'success';
						txt = 'La Tarea ha sido guardada con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
