		...
		function load(page){
			...
			$("#loader").fadeIn('slow');
			$.ajax({
				url:'ajax/riesgoasociado/listar.php',
				data: parametros,
		...
		$("#xpdf").on('click', function(event){			
			...
			doc.autoTable({ 
				...
				columns: [
					{ header: ' ', dataKey: 'no' },
					{ header: 'Nombre', dataKey: 'RIA_Nombre' },
				],
				...
				didDrawPage: function (data) {					
					...
					doc.setFontSize(20);
					doc.setTextColor(40);
					doc.text("Reporte Riesgo Asociado", data.settings.margin.left+190, 40);
		...
		$( "#edit_user" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "ajax/riesgoasociado/editar.php",
				data: parametros,
				beforeSend: function(objeto){
				...
				success: function(datos){
					...
					if(msj == 'U'){
						type = 'success';
						txt = 'Riesgo Asociado ha sido actualizado con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
		...
		$( "#delete_user" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "ajax/riesgoasociado/delete.php",
				data: parametros,
				beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
