		...
		function load(page){
			...
			$("#loader").fadeIn('slow');
			$.ajax({
				url:'ajax/listar_segclientes.php',
				data: parametros,
		...
		$("#xpdf").on('click', function(event){			
			...
			doc.autoTable({ 
				...
				columns: [
					{ header: ' ', dataKey: 'no' },
					{ header: 'Nombre', dataKey: 'SegClientesName' },
				],
				...
				didDrawPage: function (data) {					
					...
					doc.setFontSize(20);
					doc.setTextColor(40);
					doc.text("Reporte de Segmento Clientes", data.settings.margin.left+190, 40);
		...
		$('#editSegClientesModal').on('show.bs.modal', function (event) {
			setTimeout(function (){
				$('#edit_name').focus()
			}, 1500)
		...
		$('#deleteSegClientesModal').on('show.bs.modal', function (event) {
		  var button = $(event.relatedTarget) // Button that triggered the modal
		  var id = button.data('id') 
		  $('#delete_id').val(id)
		})
		...
		$( "#edit_segclientes" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
					type: "POST",
					url: "ajax/editar_segclientes.php",
					data: parametros,
					 beforeSend: function(objeto){
					...
					$('#editSegClientesModal').modal('hide');
					setTimeout(function (){
						load(1);location.reload();
					}, 3000)
				  }
		...
		$( "#add_segclientes" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
					type: "POST",
					url: "ajax/guardar_segclientes.php",
					data: parametros,
					 beforeSend: function(objeto){
						$("#resultados").html("Enviando...");
					  },
		...
		$( "#delete_segclientes" ).submit(function( event ) {
		  var parametros = $(this).serialize();
			$.ajax({
					type: "POST",
					url: "ajax/delete_segclientes.php",
					data: parametros,
					 beforeSend: function(objeto){
						$("#resultados").html("Enviando...");
					  },
					...
					$("#resultados").html(datos);
					//load(1);location.reload();
					$('#deleteSegClientesModal').modal('hide');
					setTimeout(function (){
						load(1);location.reload();
					}, 3000)
				  }
