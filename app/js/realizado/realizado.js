		...
		$("#xpdf").on('click', function(event){			
			...
			doc.autoTable({ 
				...
				columns: [
					{ header: ' ', dataKey: 'no' },
					{ header: 'Nombre', dataKey: 'REA_Nombre' },
				],
				...
				didDrawPage: function (data) {					
					...
					doc.setFontSize(20);
					doc.setTextColor(40);
					doc.text("Reporte de Realizado", data.settings.margin.left+190, 40);
		  ...
		  var parametros = $(this).serialize();
			$.ajax({
				...
				success: function(datos){
					...
					if(msj == 'U'){
						type = 'success';
						txt = 'Realizado ha sido actualizado con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
		...
		$( "#add_debilidades" ).submit(function( event ) {
			...
			$.ajax({
				type: "POST",
				url: "ajax/realizado/guardar.php",
				data: parametros,
				beforeSend: function(objeto){
					$("#resultados").html("Enviando...");
				},
				success: function(datos){
					...
					if(msj == 'O'){
						type = 'success';
						txt = 'Realizado ha sido guardado con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
						txt = 'Ya existe un Registro grabado con el mismo Nombre.';
		  ...
		  var parametros = $(this).serialize();
			$.ajax({
				...
				success: function(datos){
					...
					if(msj == 'B'){
						type = 'success';
						txt = 'Realizado ha sido borrado con éxito.';
					}
					else if(msj == 'R'){
						type= 'error';
