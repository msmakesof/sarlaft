		...
		$("#xpdf").on('click', function(event){			
			...
			doc.autoTable({ 
				...
				columns: [
					{ header: ' ', dataKey: 'no' },
					{ header: 'Nombre', dataKey: 'ACC_Nombre' },
				...
				didDrawPage: function (data) {					
					...
					doc.setFontSize(20);
					doc.setTextColor(40);
					doc.text("Reporte de Acciones", data.settings.margin.left+190, 40);
		...
		$( "#edit_user" ).submit(function( event ) {
			...
			$.ajax({
				...
				success: function(datos){
					...
					if(msj == 'U'){
						type = 'success';
						txt = 'La '+ tabla +' ha sido actualizada con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
		...
		$( "#add_user" ).submit(function( event ) {
			...
			$.ajax({
				...
				success: function(datos){
					...
					if(msj == 'O'){
						type = 'success';
						txt = 'La '+ tabla +' ha sido guardado con éxito.';
					}
					else if(msj == 'E'){
						type= 'warning';
		...
		$( "#delete_user" ).submit(function( event ) {
			...
			$.ajax({
				...
				success: function(datos){
					...
					if(msj == 'B'){
						type = 'success';
						txt = 'La '+ tabla +' ha sido eliminado con éxito';
					}
					else if(msj == 'D'){
						type= 'error';
						txt ='Error Desconocido.';
