...
?>
				<tr>
					...
					<th class='text-left'>Nombre Tipo Riesgo </th>						
					<th class='text-left'>Acciones</th>						
				</tr>
			</thead>
			<tbody>	
					<?php
					include '../../curl/tiposriesgo/listar.php';
					...
					else
					{
						if( $data["itemCount"] > 0)
						{
							...
							{
								$id=$data['body'][$i]['TIR_IdTipoRiesgo'];
								$CustomerKey=$data['body'][$i]['TIR_CustomerKey'];
								$TipoRiesgoKey=$data['body'][$i]['TIR_TipoRiesgoKey'];
								$Name= trim($data['body'][$i]['TIR_Nombre']);
								$UserKey=$data['body'][$i]['TIR_UserKey'];
					?>	
							<tr>
