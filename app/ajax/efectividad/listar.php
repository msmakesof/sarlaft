...
?>
			...
			<tbody>	
					<?php
					include '../../curl/efectividad/listar.php';
					...
					else
					{
						if( $data["itemCount"] > 0)
						{
							...
							{
								$id=$data['body'][$i]['EFE_IdEfectividad'];
								$CustomerKey=$data['body'][$i]['EFE_CustomerKey'];
								$TipoRiesgoKey=$data['body'][$i]['EFE_TipoRiesgoKey'];
								$Name= trim($data['body'][$i]['EFE_Nombre']);
								$UserKey=$data['body'][$i]['EFE_UserKey'];
					?>	
							<tr>
