...
?>
			...
			<tbody>	
					<?php
					$CustomerKey = trim($_SESSION['Keyp']);
					include '../../curl/consecuencia/listar_eveall.php';
					...
					else
					{
						if( $data["itemCount"] > 0)
						{
							...
							{
								$id=$data['body'][$i]['CSC_IdConsecuencia'];
								$escala=$data['body'][$i]['CSC_Escala'];
								$color=$data['body'][$i]['CSC_Color'];
								$CustomerKey=trim($data['body'][$i]['CSC_CustomerKey']);
								$TipoRiesgoKey=$data['body'][$i]['CSC_TipoRiesgoKey'];
								$Name= trim($data['body'][$i]['CSC_Nombre']);
								$UserKey=$data['body'][$i]['CSC_UserKey'];
					?>	
							<tr>
								...
								<td class='text-left'>
								<?php if( $modificar == 1 ) { ?>
									<a href="#" data-target="#editConsecuenciaModal" class="edit" data-toggle="modal" data-name="<?php echo $Name; ?>" data-escala="<?php echo $escala; ?>" data-color="<?php echo $color; ?>" data-ck="<?php echo $CustomerKey; ?>" data-id="<?php echo $id; ?>">
