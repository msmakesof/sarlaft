...
?>
			...
			<tbody>	
					<?php
					$CustomerKey = trim($_SESSION['Keyp']);
					include '../../curl/frecuencia/listar_eveall.php';
					...
					else
					{
						if( $data["itemCount"] > 0)
						{
							...
							{
								$id=$data['body'][$i]['FRE_IdFrecuencia'];
								$CustomerKey=trim($data['body'][$i]['FRE_CustomerKey']);
								$FrecuenciaKey=$data['body'][$i]['FRE_FrecuenciaKey'];
								$Name= trim($data['body'][$i]['FRE_Nombre']);
								$UserKey=$data['body'][$i]['FRE_UserKey'];
					?>	
							<tr>
								...
								<td class='text-left'>
								<?php if( $modificar == 1 ) { ?>
									<a href="#" data-target="#editFrecuenciaModal" class="edit" data-toggle="modal" data-name="<?php echo $Name; ?>" data-ck="<?php echo $CustomerKey; ?>" data-id="<?php echo $id; ?>">
										<i class="material-icons" data-toggle="tooltip" title="Editar" >&#xE254;</i>
									</a>
								<?php } 
								if( $eliminar == 1 ) { ?>
									<a href="#deleteFrecuenciaModal" class="delete" data-toggle="modal" data-id="<?php echo $id;?>">
