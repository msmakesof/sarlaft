...
?>
			...
			<tbody>	
					<?php
					$CustomerKey = trim($_SESSION['Keyp']);
					include '../../curl/fortalezas/listar_eveall.php';
					...
					else
					{
						if( $data["itemCount"] > 0)
						{
							...
							for($i=0; $i<count($data['body']); $i++)
							{
								...
								$Name= trim($data['body'][$i]['FortalezasName']);
					...
					?>	
							<tr>
								...
								<td class='text-left'>
								<?php if( $modificar == 1 ) { ?>
									<a href="#" data-target="#editFortalezasModal" class="edit" data-toggle="modal" data-name="<?php echo $Name; ?>" data-ck="<?php echo $CustomerKey; ?>" data-id="<?php echo $id; ?>">
										<i class="material-icons" data-toggle="tooltip" title="Editar" >&#xE254;</i>
									</a>
								<?php } 
								if( $eliminar == 1 ) { ?>
									<a href="#deleteFortalezasModal" class="delete" data-toggle="modal" data-id="<?php echo $id;?>">
