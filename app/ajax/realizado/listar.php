...
?>
			...
			<tbody>	
					<?php
					include '../../curl/realizado/listar.php';
					...
					else
					{
						if( $data["itemCount"] > 0)
						{
							...
							{
								$id=$data['body'][$i]['REA_IdRealizado'];
								$CustomerKey=$data['body'][$i]['REA_CustomerKey'];
								$TipoRiesgoKey=$data['body'][$i]['REA_TipoRiesgoKey'];
								$Name= trim($data['body'][$i]['REA_Nombre']);
								$UserKey=$data['body'][$i]['REA_UserKey'];
					?>	
							<tr>
