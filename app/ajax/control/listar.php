...
?>
			...
			<tbody>	
					<?php
					include '../../curl/control/listar.php';
					...
					else
					{
						if( $data["itemCount"] > 0)
						{
							...
							{
								$id=$data['body'][$i]['CON_IdControl'];
								$CustomerKey=$data['body'][$i]['CON_CustomerKey'];
								$TipoRiesgoKey=$data['body'][$i]['CON_TipoRiesgoKey'];
								$Name= trim($data['body'][$i]['CON_Nombre']);
								$UserKey=$data['body'][$i]['CON_UserKey'];
					?>	
							<tr>
