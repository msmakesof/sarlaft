...
$("#addtraupd").on('click', function(){
	...
	$.ajax({
		async: false,
		type: "POST",
		url: "../api/tratamientos/insertUpd.php",
		data: prmts,
...
function fnTrataUpd(ParId, ParReg){
	var itemcontrol = ParId.name;
	//alert('it trata0....'+itemcontrol);
	itemcontrol = itemcontrol.substr(11);
	//alert('it trata1....'+itemcontrol);
...
function deleteUpd(numtratamiento, eventoriesgo) {	
	//alert('numtratamiento....'+numtratamiento+'     er...'+eventoriesgo );
	//$(this).parent('td').parent('tr').remove();
	let nt = numtratamiento
	let er = eventoriesgo
	$.ajax({
		async: true,
		type: "POST",
		url: "../api/tratamientos/deleteUpd.php",
		...
		success: function(datos){
			...
			if(msj == 'S'){
				type = 'success';
				txt = 'Tratamiento ha sido borrado con éxito.';
			}
			else {
				type= 'warning';
