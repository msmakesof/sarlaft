...
if( !isset($_POST['id']) || $_POST['id'] == "" ){
    header("Location: listainter.php ");
}
...
$CustomerName = trim($reg['CustomerName']);
...
$query_empresa=sqlsrv_query($conn,"SELECT count(id) AS TotalEventos FROM GestiondeRiesgoSarlaft WHERE CustomerKey=".$_SESSION['Keyp']); // AND GER_IdInterseccion = $idreg");
$reg=sqlsrv_fetch_array($query_empresa);
$TotalEventos = $reg['TotalEventos'] ;

include("../curl/interseccion/idinterseccion.php");

$INT_IdInterseccion = $datainter['INT_IdInterseccion'];
$INT_Filas = $datainter['INT_Filas'];
$INT_Columnas = $datainter['INT_Columnas'];
$INT_CustomerKey = $datainter['INT_CustomerKey'];
$INT_USerKey = $datainter['INT_USerKey'];
$INT_InterseccionKey = $datainter['INT_InterseccionKey'];
$DateStamp = $datainter['DateStamp'];
echo "filas......$INT_Filas";
?>
<!DOCTYPE html>
<html lang="en">
								...
								<form id="formap">
									...
									<div class="form-group row" style="text-align:center">
										<div class="col-md-4">
                                            Número de Filas:
											<select id="fils">
												<option value="" <?php if( $INT_Filas == "" ){ echo "selected=selected";} ?>>Seleccione opción</option>
												<option value="1" <?php if( $INT_Filas == 1 ){ echo "selected=selected";} ?>>1</option>
												<option value="2" <?php if( $INT_Filas == 2 ){ echo "selected=selected";} ?>>2</option>
												<option value="3" <?php if( $INT_Filas == 3 ){ echo "selected=selected";} ?>>3</option>
												<option value="4" <?php if( $INT_Filas == 4 ){ echo "selected=selected";} ?>>4</option>
												<option value="5" <?php if( $INT_Filas == 5 ){ echo "selected=selected";} ?>>5</option>
											</select>
										</div>
										<div class="col-md-4">
                                            Número de Columnas:
											<select id="cols">
												<option value="" <?php if( $INT_Columnas == "" ){ echo "selected=selected";} ?>>Seleccione opción</option>
												<option value="1" <?php if( $INT_Columnas == 1 ){ echo "selected=selected";} ?>>1</option>
												<option value="2" <?php if( $INT_Columnas == 2 ){ echo "selected=selected";} ?>>2</option>
												<option value="3" <?php if( $INT_Columnas == 3 ){ echo "selected=selected";} ?>>3</option>
												<option value="4" <?php if( $INT_Columnas == 4 ){ echo "selected=selected";} ?>>4</option>
												<option value="5" <?php if( $INT_Columnas == 5 ){ echo "selected=selected";} ?>>5</option>
											</select>
										</div>
										<div class="col-md-4">
                                    ...
                                    <div class="form-group row">
										<div class="col-md-12" style="text-align:center">
                                            <div id="filcolreal" style="text-align:center !important">
                                                <?php include("../curl/interseccion/idinterseccionmatriz.php"); ?>
                                            </div>
										</div>
									</div>
        ...
        $(document).ready(function(){
			...
			$("#creatabla").on('click', function(){
				...
				if(fil == "" || col == "")
				{
                    ...
                    $("#filcol").html('')
                    if(  (fil != <?php echo $INT_Filas; ?> || col != <?php echo $INT_Columnas; ?> ) && ( <?php echo $TotalEventos; ?> > 0) )
                    {
                        //let m= result.trim()
                        ...
                        swal({
                            position: 'top-end',
                            type: 'warning',
                            title: 'Atención: Matriz no se puede modificar en cantidad de Filas y Columnas porque ya se encuentra registra en un Evento de Riesgo.',
                            showConfirmButton: true,
                            timer: 5000
                        })
                    }
                    else
                    {
                        ...
                        $.post("tablaint.php", {f: fil, c: col, idreg: <?php echo $_POST['id']; ?> }, function(result){
                            $("#filcol").html(result)
                        })
                    }
                }
            ...
            //$('#creatabla').click(); // Carga la tabla de forma automática
				...
				if(fils == "" || cols == "")
				{
					...
					if( controlcolor == "S"){
						...
						$.ajax({
							//dataType:'json',
							type: "POST",
							url: "../ajax/planes/updateinterseccion.php",
							//data:{'datos': JSON.stringify(mss)},
							data:{ 'datos': mss, 'idreg': <?php echo $_POST['id']; ?> },
							beforeSend: function(objeto){
							},
							success: function(datos){
								//alert(datos);
								...
								if( msj == "S"){
									type="success";
									title="Matriz ha sido actualizada correctamente.";
								}
                                if( msj == "E"){
									type="success";
									title="No se puede modificar la Matriz porque esta siendo usada en un Evento de Riesgo.";
								}
								swal({
									position: 'top-end',
									type: ''+type,
