...
{
	$url = $urlServicios."api/controles/lista_eve.php?ck=$CustomerKey";
	echo "url...$url<br>";
	$resultado="";
	$ch = curl_init();
	...
	foreach($datacontrol as $key => $row) {}
	
	if( $key == "message")
	{
		echo $datacontrol["message"];
	}
	else
	{
		$IdItemcsc="";
		$sel_csc="<select class='form-control' id='control' name='control' required>";
		$sel_csc.="<option value=''>Seleccione opción</option>";
		for($i=0; $i<count($datacontrol['body']); $i++)
		{				
			$condicsc = "";
			$idcsc = $datacontrol['body'][$i]["id"];
			$nombrecsc = trim($datacontrol['body'][$i]["ControlesName"]);
			$ck = trim($datacontrol['body'][$i]["CustomerKey"]);
...
<table class="table table-bordered" style="width:100% !important" id="tabctr">
	...
	<tbody id="tbody">
		<tr id="CTR-<?php echo $IdEventoRiesgo; ?>">
			<td colspan="3">				
				<table id="controlinterna" style="width:100%">					
					<tr>
						<td  style="width:100%">
							<div style="width:100%; float:left">
								<select class="form-control control" id="control" name="control">
									<?php include("../api/controles/lista_eve.php"); 
										$resultado="";
										$ch = curl_init();
										curl_setopt($ch, CURLOPT_VERBOSE, true);
										curl_setopt($ch, CURLOPT_URL, $url);
										...
										else
										{
											...
											{				
												$condicontrol = "";
												$idcontrol = $datacontrol['body'][$i]["id"];
												$nombrecontrol = trim($datacontrol['body'][$i]["ControlesName"]);
												$ck = trim($datacontrol['body'][$i]["CustomerKey"]);
												$cuasak = trim($datacontrol['body'][$i]["UserKey"]);
												if( isset($IdItemcsc) && $IdItemcsc != "" && $idcontrol == $IdItemcsc ){
													$condicontrol = ' selected="selected" ';
												}
										?>
												<option value="<?php echo $idcontrol . $condicontrol ;?>"><?php echo $nombrecontrol;?></option>
										<?php
											}
										}
